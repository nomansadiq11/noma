...
def _get_pod_logs(core: client.CoreV1Api, namespace: str, pod_name: str, container_name: str, previous: bool = False) -> str | None:
...
def _classify_pod_state(pod: Any) -> dict[str, Any]:
    ...
    conditions = {c.type: c.status for c in (pod.metadata.managed_fields or []) if hasattr(c, "type")}
    ...
    if pod.status.container_statuses:
        for status in pod.status.container_statuses:
            ...
            if (status.restart_count or 0) > 3:
                if "Restart" not in classification["reason"]:
                    classification["reason"] += f" (high restart count: {status.restart_count})"

    return classification


def _check_node_capacity(core: client.CoreV1Api, apps: client.AppsV1Api, namespace: str, pod: Any) -> dict[str, Any]:
    ...
    try:
        ...
        for node in nodes.items:
            ...
            # Check allocatable vs pod request
    ...
    try:
        ...
        for quota in quotas.items:
            ...
            for resource, hard_val in hard.items():
                ...
                try:
                    hard_int = _parse_resource_quantity(hard_val) if resource == "memory" else _parse_cpu_quantity(hard_val) if resource == "cpu" else int(hard_val)
                    used_int = _parse_resource_quantity(used_val) if resource == "memory" else _parse_cpu_quantity(used_val) if resource == "cpu" else int(used_val)
...
def _rank_hypotheses(pod_state: dict[str, Any], pod_events: dict[str, Any], node_info: dict[str, Any], pod: Any) -> list[dict[str, Any]]:
    """Rank root cause hypotheses with confidence scores."""
    hypotheses = []

    # Pending pod with FailedScheduling
    if pod_state["state_type"] == "Pending" and "FailedScheduling" in pod_events:
        ...
        if "Insufficient" in event_msg or "insufficient" in event_msg:
            ...
            hypotheses.append({
                ...
                "confidence": "high",
                "evidence": [
                    f"Pod event: FailedScheduling - taints mismatch",
                    f"Message: {event_msg}",
                ],
    ...
    # CrashLoopBackOff with previous logs
...
def diagnose(payload: DiagnoseRequest) -> DiagnoseResponse:
    ...
    # Deep diagnostic path for error checks
    if error_check_intent or deployments.items or pods.items:
        ...
        for pod in pods.items[:40]:
            ...
            node_info = _check_node_capacity(core, apps, namespace, pod)
            ...
            # Rank hypotheses
            hypotheses = _rank_hypotheses(pod_state, pod_events, node_info, pod)
        ...
        if hypotheses_list:
            ...
            if len(hypotheses_list) > 1:
                answer += f"**Alternative hypotheses:**\n"
    ...
    if not pods.items:
        ...
        answer = (
            ...
            f"To get detailed diagnostics including logs and events, ask about specific issues."
        )
        evidence.append({"tool": "pod_summary", "items": pod_details})
